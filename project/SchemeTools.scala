...
object SchemeTools {
  ...
  private val Schemes = SchemeProps("fut_info.ini", "FutInfo", "CustReplScheme") ::
    SchemeProps("opt_info.ini", "OptInfo", "CustReplScheme") :: Nil

  def generateSchemes(projectDir: File, outDir: File): Seq[File] = {
    val temp = IO.createTemporaryDirectory

    Schemes.foreach {case props: SchemeProps =>
      val cmd = makeCmd(
        temp / props.fileName,
        props.className,
