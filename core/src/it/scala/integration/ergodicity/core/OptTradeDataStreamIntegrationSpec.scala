...
class OptTradeDataStreamIntegrationSpec extends TestKit(ActorSystem("OptTradeDataStreamIntegrationSpec", ConfigWithDetailedLogging)) with ImplicitSender with WordSpec {
  ...
  "DataStream" must {
    "do some stuff" in {
      ...
      dataStream ! BindTable("orders_log", ordersLogRepo, implicitly[Deserializer[OptTrade.OrdersLogRecord]])
      dataStream ! BindTable("deal", dealsRepo, implicitly[Deserializer[OptTrade.DealRecord]])
